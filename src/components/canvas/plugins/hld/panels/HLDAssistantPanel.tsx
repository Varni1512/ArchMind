...
export function HLDAssistantPanel({ excalidrawAPI, onClose }: Props) {
  const { currentQuestion, loadedHistory } = useHLDWorkspace();
  const diagramType = currentQuestion?.recommendedDiagramType || 'System Architecture';
  ...
  // Evaluation Hook
  const { loading, retryCount, error, evaluation, evaluateDesign } = useHLDAIEvaluation(excalidrawAPI, diagramType, currentQuestion?.id);
  ...
  const handleSaveHistory = async () => {
    ...
    try {
      const { HLDASTEngine } = await import('../ast/HLDASTEngine');
      ...
      const response = await fetch('/api/ai/hld-history', {
        method: 'POST',
    ...
    <div className="w-[400px] h-full bg-surface border-l border-primary/10 flex flex-col shrink-0 shadow-xl">
      <div className="p-4 border-b border-primary/10 flex flex-col gap-4 bg-surface relative z-10 shrink-0">
        <div className="flex justify-between items-start">
          <div>
            <h2 className="font-heading font-bold text-lg text-primary-ink">HLD Assistant</h2>
            <p className="text-xs text-primary/60 mt-1">AI-powered architecture feedback</p>
          </div>
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          <div className="p-4 bg-surface border-b border-primary/5 flex justify-end gap-2">
            ...
            <button
              ...
              className="bg-purple-600 hover:bg-purple-700 text-white px-3 py-1.5 rounded-lg text-sm font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 cursor-pointer"
            >
              {loading ? (
                <>
          ...
          >
            ...
            {evaluation && !loading && (
              <div className="space-y-5 animate-in fade-in duration-300">
                ...
                <div className="bg-primary/5 rounded-xl p-4 border border-primary/10">
                  ...
                  {evaluation.scoreBreakdown && Object.keys(evaluation.scoreBreakdown).length > 0 && (
                    <div className="space-y-2 mt-3 pt-3 border-t border-primary/10">
                      {Object.values(evaluation.scoreBreakdown).map((item: any, idx: number) => (
                ...
                {/* Improvements */}
                {evaluation.improvements && Object.values(evaluation.improvements).some((arr: any) => arr && arr.length > 0) && (
                  <div>
                    <h3 className="font-semibold text-sm text-primary-ink mb-3 flex items-center gap-2">
                    ...
                    <ul className="space-y-2">
                      {evaluation.improvements.highPriority?.map((item: any, idx: number) => (
                        <li key={`high-${idx}`} className="flex items-start gap-2 text-sm text-primary/80">
                          <span className="w-1.5 h-1.5 rounded-full bg-red-400 mt-1.5 shrink-0" />
                          <span>{item}</span>
                        </li>
                      ))}
                      {evaluation.improvements.mediumPriority?.map((item: any, idx: number) => (
                        <li key={`med-${idx}`} className="flex items-start gap-2 text-sm text-primary/80">
                          <span className="w-1.5 h-1.5 rounded-full bg-amber-400 mt-1.5 shrink-0" />
                          <span>{item}</span>
                        </li>
                      ))}
                      {evaluation.improvements.lowPriority?.map((item: any, idx: number) => (
          ...
          <div className="p-4 bg-surface border-t border-primary/10 shadow-[0_-4px_10px_rgba(0,0,0,0.02)] z-10">
            <form onSubmit={sendMessage} className="relative">
              ...
              <button
                ...
                className="absolute right-1.5 top-1/2 -translate-y-1/2 p-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors cursor-pointer"
              >
                <Send size={16} />
              </button>
            </form>
